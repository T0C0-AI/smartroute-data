...
**진행 상황: 66 / 226 지역 완료 (2026-08-09 기준)**. 남은 지역은 다음 실행에서 이어간다.
...
- 이름이 우리 데이터와 유일하게(중복 없이) 일치하는 곳만 반영. **전국 226/226 지역 완료
  (2026-08-09 기준)** — KorService2(주차 등)와 별도 API 상품이라 호출 한도가 따로
  있어서, 같은 날에 이쪽만 먼저 다 끝났다.
